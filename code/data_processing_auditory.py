...
def process_auditory_data(input_csv_file, start_index, end_index, text_list, output_dir):
    ...
    def get_file_path(row):
        return f"../testdata/{row['path']}/{row['file_id']}.mp3"
    ...
    try:
        ...
        inputs = {
            ...
            ModalityType.AUDIO: data.load_and_transform_audio_data(file_paths, device)
        }

        with torch.no_grad():
            embeddings = model(inputs)

        conformance_check = torch.softmax(embeddings[ModalityType.AUDIO] @ embeddings[ModalityType.TEXT].T, dim=-1)
